    ...
    if ($link = db_connect()) { // connect to DB
        foreach ($changeTo as $coin) {
            ...
            if ($result && mysqli_num_rows($result) > 0) { // if line exists and up to date
                ...
                // API call with CURL
                $handle = curl_init();
                $url = "https://api.exchangeratesapi.io/latest?base=$changeFrom&symbols=$changeTo";
                ...
                if(curl_exec($handle)){
                    ...
                    $currencies = get_object_vars($currencies); // gets the properties of the given object
